...
// function requireAccount(accountAddress: string): Account {
...
//     fpmmTrade.transactionHash = transactionHash.toHexString();
...
//     fpmmTransaction.transactionHash = transactionHash.toHexString();
...
export function handleFundingRemoved(event: FPMMFundingRemoved): void {
  ...
  // }
  // let collateral = requireToken(fpmm.collateralToken as Bytes);
  // let collateralScale = collateral.scale;
  // let collateralScaleDec = collateralScale.toBigDecimal();
  // let ethPerCollateral = collateral.ethPerToken;
  // let usdPerEth = requireGlobal().usdPerEth;
...
export function handleBuy(event: FPMMBuyEvent): void {
  ...
  let agent = TraderAgent.load(event.params.buyer.toHexString());
  if (agent !== null) {
    
    agent.buyAmount =  agent.buyAmount.plus(event.params.investmentAmount)
    agent.buyAmountFee = agent.buyAmountFee.plus(event.params.feeAmount)
    agent.save();
  }
}
...
export function handleSell(event: FPMMSellEvent): void {
  ...
  updateTraderAgent(
    event.params.seller,
    event.block.timestamp,
    BigInt.fromI32(0).minus(returnAmountPlusFees),
    BigInt.fromI32(0),
    1,0,0,0
  );

  let agent = TraderAgent.load(event.params.seller.toHexString());
  if (agent !== null) {
    
    agent.sellAmount =  agent.sellAmount.plus(event.params.returnAmount)
    agent.sellAmountFee = agent.sellAmountFee.plus(event.params.feeAmount)
    agent.save();
  }
}
